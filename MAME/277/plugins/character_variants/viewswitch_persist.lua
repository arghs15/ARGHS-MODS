...
function lib:load_plugin_settings()
	local plugin_settings = { config_mode = 'global' }  -- default to global
			...
			for i_str, target_excludes in pairs(exclude_settings) do
				...
				if target then
					...
					for j, view_name in pairs(target_excludes) do
						for k, v in pairs(target.view_names) do
							if view_name == v then
...
-- Internal function to save excluded views to a specific file type
function lib:save_excluded_views_to_file(excluded_views, save_global)
	...
	-- if nothing to save, remove existing exclude file
	...
	for target_index, target_excludes in pairs(excluded_views) do
		...
		if has_excludes then break end
	end
	
	if not has_excludes then
		os.remove(filename)
	else
		if not stat then
			lfs.mkdir(path)
		...
		-- flatten the exclude settings
		local exclude_settings = { }
		local render_targets = manager.machine.render.targets
		for target_index, target_excludes in pairs(excluded_views) do
			...
			if target then
				...
				for view_index, excluded in pairs(target_excludes) do
					if excluded then
						table.insert(excluded_list, target.view_names[view_index])
					end
				end
				if #excluded_list > 0 then
					exclude_settings[target_index] = excluded_list
				end
			end
		end
		...
		-- try to write the file
		local json = require('json')
		local text = json.stringify(exclude_settings, { indent = true })
		local file = io.open(filename, 'w')
		if not file then
...
function lib:save_settings(switch_hotkeys, cycle_hotkeys, config_mode)
	-- Use the internal function based on config mode
	local save_global = (config_mode == 'global')
	if config_mode == 'rom_specific' then
		save_global = false
	elseif config_mode == 'global_with_overrides' then
		-- In override mode, always save to ROM-specific to create/update override
		save_global = false
	end
