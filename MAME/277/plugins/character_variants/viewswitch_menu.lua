...
local function populate_exclude()
	...
	table.insert(items, { 'Exclude Views from Cycle', '', 'off' })
	...
	if #targets == 0 then
		...
		for i, target in pairs(targets) do
			...
			for j, view in pairs(target.view_names) do
				local flags = ''
				if excluded_views[target.index] and excluded_views[target.index][j] then
					flags = 'lr'
				end
				local status = (excluded_views[target.index] and excluded_views[target.index][j]) and 'Excluded' or 'Included'
				table.insert(items, { view, status, flags })
			end
		end
	end
...
local function handle_settings(index, event)
	...
	elseif (event == 'select') and (index == settings_done) then
		...
		if pending_config_mode and pending_config_mode ~= plugin_settings.config_mode then
			...
			if old_config_mode == 'global' and pending_config_mode == 'rom_specific' then
				-- Copy global settings to ROM but keep global files
			...
			elseif old_config_mode == 'rom_specific' and pending_config_mode == 'global' then
				-- Save current ROM settings back to global
				persister:save_settings_to_file(switch_hotkeys, cycle_hotkeys, true)  -- true = global
				persister:save_excluded_views_to_file(excluded_views, true)  -- true = global
			else
				-- For other transitions or global_with_overrides, use normal save with old mode
			...
			for k, hotkey in pairs(new_cycle) do
				hotkey.pressed = false  -- ensure clean state
				table.insert(cycle_hotkeys, hotkey)
			end
			-- Rebuild excluded_views properly
	...
	elseif event == 'select' then
		if index == settings_config_mode_index then  -- toggle config_mode setting
			-- Cycle through modes but don't apply immediately
			...
			return true
		elseif index == settings_remove_overrides_index then  -- Remove ROM overrides option
			-- This action should be immediate since it's destructive
			if plugin_settings.config_mode == 'global_with_overrides' then
				...
				-- load global settings (since overrides are now removed)
				local new_switch, new_cycle = persister:load_settings(plugin_settings.config_mode)
				local new_excluded = persister:load_excluded_views(plugin_settings.config_mode)
...
local function populate_settings()
	...
	local settings_info = persister:get_current_settings_info(plugin_settings.config_mode)  -- Always use current mode for file status
	...
	if pending_config_mode and pending_config_mode ~= plugin_settings.config_mode then
		...
		mode_flags = 'lr'  -- Highlight the changed setting
	end
	
	table.insert(items, { 'Configuration mode', config_description, mode_flags })
	settings_config_mode_index = #items  -- Store the index for this selectable item
	table.insert(items, { '---', '', '' })
	...
	if plugin_settings.config_mode == 'global_with_overrides' and not pending_config_mode then
		...
		if settings_info.has_rom_settings or settings_info.has_rom_excludes then
			...
			settings_remove_overrides_index = #items  -- Store the index for this selectable item
		end
	end
...
function lib:init(switch, cycle, exclude, settings)
	...
	plugin_settings = settings or { config_mode = 'global' }
