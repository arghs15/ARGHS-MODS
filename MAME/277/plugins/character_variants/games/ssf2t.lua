-- SSF2T Game Module for Character Variants Plugin
local ssf2t = {}

-- Character variants for SSF2T with separate P1/P2 states (LARGEST offsets in the series!)
...
-- Memory addresses for SSF2T (based on cheat file)
...
-- Variant override memory - using different range for SSF2T
local variant_memory = {
    p1_override_active = 0xFFFF70,
    p1_override_state = 0xFFFF71,
    p2_override_active = 0xFFFF72,
    p2_override_state = 0xFFFF73,
}
...
-- Get current characters (based on SSF2T memory layout)
...
local function clear_variant_override(player)
    ...
    if pcall(function() 
        ...
        print(string.format("SSF2T: Cleared P%d variant override", player))
    else
        print(string.format("SSF2T: Failed to clear P%d variant override", player))
    end
end
...
local function trigger_variant_display(player, variant_state)
    ...
    if success then
        print(string.format("SSF2T: P%d variant set to 0x%02X", player, variant_state))
        return true
    else
        print(string.format("SSF2T: P%d variant write failed", player))
        return false
    end
end
...
local function cycle_character_variant(player, character_id)
    if not character_variants[character_id] then
        print(string.format("SSF2T: P%d character 0x%02X has no variants", player, character_id))
        return
    end
    ...
    if #states <= 1 then
        if #states == 0 then
            print(string.format("SSF2T: P%d %s has no image files available", 
                  player, char_data.name))
        else
            print(string.format("SSF2T: P%d %s has only one variant", 
                  player, char_data.name))
        end
        return
    end
    ...
    print(string.format("SSF2T: P%d %s variant %d/%d (state 0x%02X)", 
          player, char_data.name, 
...
-- Module API
function ssf2t.init()
    print("SSF2T module initialized")
    p1_current_variant = {}
    p2_current_variant = {}
    ...
    if manager and manager.machine then
        ...
        if cpu then
            ...
            if mem then
                pcall(function()
                    for i = 0, 15 do
                        mem:write_u8(0xFFFF70 + i, 0)
                    end
                    print("SSF2T: Cleared variant override memory")
                end)
            end
        end
    end
    ...
    if p1_char ~= p1_last_character then
        if p1_last_character and p1_last_character ~= 0x20 and
           ...
           p1_char ~= p1_last_character then
            print(string.format("SSF2T: P1 switched from 0x%02X to 0x%02X - clearing override", p1_last_character, p1_char))
    ...
    if p2_char ~= p2_last_character then
        if p2_last_character and p2_last_character ~= 0x20 and
           ...
           p2_char ~= p2_last_character then
            print(string.format("SSF2T: P2 switched from 0x%02X to 0x%02X - clearing override", p2_last_character, p2_char))
...
function ssf2t.cycle_p1_variant()
    ...
    if manager and manager.machine then
        ...
        if cpu then
            ...
            if mem then
                ...
                print(string.format("SSF2T: P1 actual memory char: 0x%02X, akuma: %d, function returned: 0x%02X", 
    ...
    print(string.format("SSF2T: P1 character detected as 0x%02X", p1_char or 0))
    if p1_char and p1_char ~= 0x20 and (p1_char <= 0x0F or p1_char == 0x10) then
        cycle_character_variant(1, p1_char)
    else
        print("SSF2T: P1 character is invalid or not detected")
    end
end

function ssf2t.cycle_p2_variant()
    ...
    if manager and manager.machine then
        ...
        if cpu then
            ...
            if mem then
                ...
                print(string.format("SSF2T: P2 actual memory char: 0x%02X, akuma: %d, function returned: 0x%02X", 
    ...
    print(string.format("SSF2T: P2 character detected as 0x%02X", p2_char or 0))
    if p2_char and p2_char ~= 0x20 and (p2_char <= 0x0F or p2_char == 0x10) then
        cycle_character_variant(2, p2_char)
    else
        print("SSF2T: P2 character is invalid or not detected")
    end
end

function ssf2t.cycle_both_variants()
    ssf2t.cycle_p1_variant()
    ssf2t.cycle_p2_variant()
end

function ssf2t.cleanup()
    print("SSF2T module cleanup")
    p1_current_variant = {}
    p2_current_variant = {}
    p1_last_character = nil
    p2_last_character = nil
...
function ssf2t.debug_state()
    if not manager or not manager.machine then
        print("SSF2T: No manager or machine available")
        return
    end
    ...
    local cpu = machine.devices[":maincpu"]
    if not cpu then
        print("SSF2T: No CPU device")
        return
    end
    
    local mem = cpu.spaces["program"]
    if not mem then
        print("SSF2T: No memory space")
        return
    end
    ...
    print(string.format("SSF2T Debug - Timer: %d, P1: 0x%02X(Akuma:%d), P2: 0x%02X(Akuma:%d), P1 Energy: %d, P2 Energy: %d", 
