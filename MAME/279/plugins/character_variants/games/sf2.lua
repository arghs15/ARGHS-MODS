-- SF2 Game Module for Character Variants Plugin
local sf2 = {}

-- Character variants for SF2 with separate P1/P2 states
local character_variants = {
    ...
    [0x03] = {
        ...
        p2_states = {0x03, 0x32}   -- Guile 01 + Guile 02 (P2)
    },
    ...
    -- Chun Li: 1 variant (only Chun Li 01 available)
    [0x05] = {
        name = "chun_li",
        p1_states = {0x05},  -- Chun Li 01 only
        p2_states = {0x05}   -- Chun Li 01 only
    },
    -- Zangief: 2 variants
    [0x06] = {
        name = "zangief",
        p1_states = {0x06, 0x31},  -- Zangief 01 + Zangief 02
        p2_states = {0x06, 0x33}   -- Zangief 01 + Zangief 02 (P2)
    },
    -- Dhalsim: 1 variant (only Dhalsim 01 available)
    [0x07] = {
        name = "dhalsim",
        p1_states = {0x07},  -- Dhalsim 01 only
        p2_states = {0x07}   -- Dhalsim 01 only
    },
    -- M. Bison: no variants (not available in names.txt)
    [0x08] = {
        name = "m_bison",
        p1_states = {},  -- No images available
        p2_states = {}   -- No images available
    },
    -- Sagat: no variants (not available in names.txt)
    [0x09] = {
        name = "sagat",
        p1_states = {},  -- No images available
        p2_states = {}   -- No images available
    },
    -- Balrog: no variants (not available in names.txt)
    [0x0A] = {
        name = "balrog",
        p1_states = {},  -- No images available
        p2_states = {}   -- No images available
    },
    -- Vega: no variants (not available in names.txt)
    [0x0B] = {
        name = "vega",
        p1_states = {},  -- No images available
        p2_states = {}   -- No images available
...
-- Memory addresses for SF2 (based on cheat file)
local memory_addresses = {
    game_timer = 0xFF8ACE,    -- Timer from "Infinite Time" cheat (16-bit)
    p1_char = 0xFF8657,       -- P1 character from "P1 Select Character" cheat
    p2_char = 0xFF8957,       -- P2 character from "P2 Select Character" cheat
    p1_energy1 = 0xFF83F0,    -- P1 energy addresses
    p1_energy2 = 0xFF8582,
    p2_energy1 = 0xFF86F0,    -- P2 energy addresses
    p2_energy2 = 0xFF8882,
    p1_rounds = 0xFF8656,     -- P1 rounds won
    p2_rounds = 0xFF8956,     -- P2 rounds won
    background = 0xFFDD7E     -- Background selector
}

-- Variant override memory - using different range for SF2
local variant_memory = {
    p1_override_active = 0xFFFF40,
    p1_override_state = 0xFFFF41,
    p2_override_active = 0xFFFF42,
    p2_override_state = 0xFFFF43,
}
...
-- Get current characters (based on SF2 memory layout)
local function get_current_characters()
    ...
    -- Always read character data - SF2 seems to always have valid character IDs
...
local function clear_variant_override(player)
    ...
    if pcall(function() 
        ...
        print(string.format("SF2: Cleared P%d variant override", player))
    else
        print(string.format("SF2: Failed to clear P%d variant override", player))
    end
end
...
local function trigger_variant_display(player, variant_state)
    if not manager or not manager.machine then 
        print("SF2: No manager or machine")
        return false
    end
    ...
    local cpu = machine.devices[":maincpu"]
    if not cpu then 
        print("SF2: No CPU device")
        return false
    end
    
    local mem = cpu.spaces["program"]
    if not mem then 
        print("SF2: No memory space")
        return false
    end
    ...
    if write_success then
        ...
        if verify_success and verify_msg then
            print(string.format("SF2: P%d variant set to 0x%02X - SUCCESS", player, variant_state))
            return true
        else
            print(string.format("SF2: P%d variant write failed verification", player))
            return false
        end
    else
        print(string.format("SF2: Memory write failed - %s", tostring(error_msg)))
        return false
    end
end
...
local function cycle_character_variant(player, character_id)
    if not character_variants[character_id] then
        print(string.format("SF2: P%d character 0x%02X has no variants", player, character_id))
        return
    end
    ...
    if #states <= 1 then
        if #states == 0 then
            print(string.format("SF2: P%d %s has no image files available", 
                  player, char_data.name))
        else
            print(string.format("SF2: P%d %s has only one variant", 
                  player, char_data.name))
        end
        return
    end
    ...
    print(string.format("SF2: P%d %s variant %d/%d (state 0x%02X)", 
          player, char_data.name, 
    ...
    if manager and manager.machine then
        ...
        if cpu then
            ...
            if mem then
                pcall(function()
                    for i = 0, 15 do
                        mem:write_u8(0xFFFF40 + i, 0)
                    end
                    print("SF2: Cleared variant override memory")
                end)
            end
        end
    end
    ...
    if p1_char ~= p1_last_character then
        if p1_last_character and p1_last_character ~= 0x20 and
           ...
           p1_char ~= p1_last_character then
            print(string.format("SF2: P1 switched from 0x%02X to 0x%02X - clearing override", p1_last_character, p1_char))
    ...
    if p2_char ~= p2_last_character then
        if p2_last_character and p2_last_character ~= 0x20 and
           ...
           p2_char ~= p2_last_character then
            print(string.format("SF2: P2 switched from 0x%02X to 0x%02X - clearing override", p2_last_character, p2_char))
...
function sf2.cycle_p1_variant()
    ...
    if manager and manager.machine then
        ...
        if cpu then
            local mem = cpu.spaces["program"]
            if mem then
                local actual_char = mem:read_u8(0xFF8657)
                print(string.format("SF2: P1 actual memory char: 0x%02X, function returned: 0x%02X", actual_char or 0, p1_char or 0))
    ...
    print(string.format("SF2: P1 character detected as 0x%02X", p1_char or 0))
    if p1_char and p1_char ~= 0x20 and p1_char <= 0x0B then
        cycle_character_variant(1, p1_char)
    else
        print("SF2: P1 character is invalid or not detected")
    end
end

function sf2.cycle_p2_variant()
    ...
    if manager and manager.machine then
        ...
        if cpu then
            local mem = cpu.spaces["program"]
            if mem then
                local actual_char = mem:read_u8(0xFF8957)
                print(string.format("SF2: P2 actual memory char: 0x%02X, function returned: 0x%02X", actual_char or 0, p2_char or 0))
    ...
    print(string.format("SF2: P2 character detected as 0x%02X", p2_char or 0))
    if p2_char and p2_char ~= 0x20 and p2_char <= 0x0B then
        cycle_character_variant(2, p2_char)
    else
        print("SF2: P2 character is invalid or not detected")
    end
end

function sf2.cycle_both_variants()
    sf2.cycle_p1_variant()
    sf2.cycle_p2_variant()
end

function sf2.cleanup()
    print("SF2 module cleanup")
    p1_current_variant = {}
    p2_current_variant = {}
    p1_last_character = nil
    p2_last_character = nil
...
function sf2.debug_state()
    if not manager or not manager.machine then
        print("SF2: No manager or machine available")
        return
    end
    ...
    print(string.format("SF2 Debug - Timer: %d, P1: 0x%02X, P2: 0x%02X, P1 Energy: %d, P2 Energy: %d", 
