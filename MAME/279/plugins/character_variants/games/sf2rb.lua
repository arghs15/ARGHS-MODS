-- SF2CE Game Module for Character Variants Plugin
local sf2ce = {}

-- Character variants for SF2CE with separate P1/P2 states
local character_variants = {
    ...
    [0x01] = {
        name = "e_honda",
        p1_states = {0x01, 0x34},  -- E Honda 01 + E Honda 02
        p2_states = {0x01, 0x3A}   -- E Honda 01 + E Honda 02 (P2)
    },
    ...
    [0x03] = {
        ...
        p2_states = {0x03, 0x36}   -- Guile 01 + Guile 02 (P2)
    },
    ...
    [0x05] = {
        ...
        p2_states = {0x05, 0x39}   -- Chun Li 01 + Chun Li 02 (P2)
    },
    -- Zangief: 3 variants
    [0x06] = {
        name = "zangief",
        p1_states = {0x06, 0x31, 0x32},  -- Zangief 01 + Zangief 02 + Zangief 03
        p2_states = {0x06, 0x37, 0x38}   -- Zangief 01 + Zangief 02 + Zangief 03 (P2)
    },
    -- Dhalsim: 2 variants
    [0x07] = {
        name = "dhalsim",
        p1_states = {0x07, 0x35},  -- Dhalsim 01 + Dhalsim 02
        p2_states = {0x07, 0x3B}   -- Dhalsim 01 + Dhalsim 02 (P2)
    },
    -- M. Bison: 1 variant
    [0x08] = {
        name = "m_bison",
    ...
    -- Sagat: 1 variant (Zagat 01 in the layout)
    [0x09] = {
        name = "sagat",
        p1_states = {0x09},  -- Zagat 01
        p2_states = {0x09}   -- Zagat 01
    },
    -- Balrog: 1 variant
    [0x0A] = {
        name = "balrog",
...
-- Memory addresses for SF2CE (based on cheat file)
...
-- Variant override memory - using different range for SF2CE
local variant_memory = {
    p1_override_active = 0xFFFF50,
    p1_override_state = 0xFFFF51,
    p2_override_active = 0xFFFF52,
    p2_override_state = 0xFFFF53,
}
...
-- Get current characters (based on SF2CE memory layout)
local function get_current_characters()
    ...
    -- Always read character data - SF2CE seems to always have valid character IDs
...
local function clear_variant_override(player)
    ...
    if pcall(function() 
        ...
        print(string.format("SF2CE: Cleared P%d variant override", player))
    else
        print(string.format("SF2CE: Failed to clear P%d variant override", player))
    end
end
...
local function trigger_variant_display(player, variant_state)
    if not manager or not manager.machine then 
        print("SF2CE: No manager or machine")
        return false
    end
    ...
    local cpu = machine.devices[":maincpu"]
    if not cpu then 
        print("SF2CE: No CPU device")
        return false
    end
    
    local mem = cpu.spaces["program"]
    if not mem then 
        print("SF2CE: No memory space")
        return false
    end
    ...
    if write_success then
        ...
        if verify_success and verify_msg then
            print(string.format("SF2CE: P%d variant set to 0x%02X - SUCCESS", player, variant_state))
            return true
        else
            print(string.format("SF2CE: P%d variant write failed verification", player))
            return false
        end
    else
        print(string.format("SF2CE: Memory write failed - %s", tostring(error_msg)))
        return false
    end
end
...
local function cycle_character_variant(player, character_id)
    if not character_variants[character_id] then
        print(string.format("SF2CE: P%d character 0x%02X has no variants", player, character_id))
        return
    end
    ...
    if #states <= 1 then
        if #states == 0 then
            print(string.format("SF2CE: P%d %s has no image files available", 
                  player, char_data.name))
        else
            print(string.format("SF2CE: P%d %s has only one variant", 
                  player, char_data.name))
        end
        return
    end
    ...
    print(string.format("SF2CE: P%d %s variant %d/%d (state 0x%02X)", 
          player, char_data.name, 
...
-- Module API
function sf2ce.init()
    print("SF2CE module initialized")
    p1_current_variant = {}
    p2_current_variant = {}
    ...
    if manager and manager.machine then
        ...
        if cpu then
            ...
            if mem then
                pcall(function()
                    for i = 0, 15 do
                        mem:write_u8(0xFFFF50 + i, 0)
                    end
                    print("SF2CE: Cleared variant override memory")
                end)
            end
        end
    end
    ...
    if p1_char ~= p1_last_character then
        if p1_last_character and p1_last_character ~= 0x20 and
           ...
           p1_char ~= p1_last_character then
            print(string.format("SF2CE: P1 switched from 0x%02X to 0x%02X - clearing override", p1_last_character, p1_char))
    ...
    if p2_char ~= p2_last_character then
        if p2_last_character and p2_last_character ~= 0x20 and
           ...
           p2_char ~= p2_last_character then
            print(string.format("SF2CE: P2 switched from 0x%02X to 0x%02X - clearing override", p2_last_character, p2_char))
...
function sf2ce.cycle_p1_variant()
    ...
    if manager and manager.machine then
        ...
        if cpu then
            ...
            if mem then
                ...
                print(string.format("SF2CE: P1 actual memory char: 0x%02X, function returned: 0x%02X", actual_char or 0, p1_char or 0))
    ...
    print(string.format("SF2CE: P1 character detected as 0x%02X", p1_char or 0))
    if p1_char and p1_char ~= 0x20 and p1_char <= 0x0B then
        cycle_character_variant(1, p1_char)
    else
        print("SF2CE: P1 character is invalid or not detected")
    end
end

function sf2ce.cycle_p2_variant()
    ...
    if manager and manager.machine then
        ...
        if cpu then
            ...
            if mem then
                ...
                print(string.format("SF2CE: P2 actual memory char: 0x%02X, function returned: 0x%02X", actual_char or 0, p2_char or 0))
    ...
    print(string.format("SF2CE: P2 character detected as 0x%02X", p2_char or 0))
    if p2_char and p2_char ~= 0x20 and p2_char <= 0x0B then
        cycle_character_variant(2, p2_char)
    else
        print("SF2CE: P2 character is invalid or not detected")
    end
end

function sf2ce.cycle_both_variants()
    sf2ce.cycle_p1_variant()
    sf2ce.cycle_p2_variant()
end

function sf2ce.cleanup()
    print("SF2CE module cleanup")
    p1_current_variant = {}
    p2_current_variant = {}
    p1_last_character = nil
    p2_last_character = nil
...
function sf2ce.debug_state()
    if not manager or not manager.machine then
        print("SF2CE: No manager or machine available")
        return
    end
    ...
    print(string.format("SF2CE Debug - Timer: %d, P1: 0x%02X, P2: 0x%02X, P1 Energy: %d, P2 Energy: %d", 
